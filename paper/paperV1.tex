...
This paper demonstrates, on a controlled dataset, that machine learning techniques based on state of the art neural architectures can predict the perceived time of presence of several sound sources with excellent performance, at a degree of precision that is sufficient
...
% to yield similar perceptual responses compared to recorded sound scenes.
...
% Though, the time of presence is not a parameter of the simulation process and thus the distribution of the scene population in this space is not controlled. As an alternative,
...
\begin{figure}[t]
    ...
    \caption{Architecture of the deep learning model used for source presence prediction.\ml{refaire la figure en vertical}}\label{fig:deep_arch}
\end{figure}
...
\hline
	...
	$\hat T_T(\alpha_{opt}, \beta_{opt})$ & -0.51** & NS & 0.33** & -0.56** & -0.27** & 0.58** & 0.82** & -0.36** & -0.33** \\
	$\hat T_V(\alpha_{opt}, \beta_{opt})$ & NS & 0.44** & NS & 0.36** & NS & -0.23* & -0.41** & 0.83** & NS \\
	$\hat T_B(\alpha_{opt}, \beta_{opt})$ & 0.58** & -0.33** & -0.47** & 0.55** & 0.52** & -0.45** & -0.56** & NS & 0.91** \\ \hline
\end{tabular}
}
\end{table*}
...
\hat P_{3, \varphi} &= 13.03 - 0.16~L_{50, 1kHz} + 9.60~TFSD_{500Hz, 1s} \\
& + 5.00~TFSD_{4kHz, 1/8s}
\end{align}
